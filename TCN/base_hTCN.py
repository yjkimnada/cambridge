...
class base_hTCN(nn.Module):
    def __init__(self, C_den, C_syn_e, C_syn_i, T_no, M_no, B_no):
        ...
        """
        self.conv1_e = CausalConv1d(in_channels = self.sub_no,
                                   out_channels = self.sub_no * self.M_no,
                                   groups = self.sub_no,
                                   kernel_size = self.T_no)
        self.conv1_i = CausalConv1d(in_channels = self.sub_no,
                                   out_channels = self.sub_no * self.M_no,
                                   groups = self.sub_no,
                                   kernel_size = self.T_no)
        """
    ...
    def forward(self, S_e, S_i):
        ...
        #S_e_conv1 = self.conv1_e(S_e)
        #S_i_conv1 = self.conv1_i(S_i)
