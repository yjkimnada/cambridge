...
class PROB_HVAE_shGLM(nn.Module):
    ...
    def encode(self, V_in, S_e, S_i):
        ...
        posterior_probs_logs = -F.softplus(NN_out + S_conv[:,1:] + self.enc_bias)
        return posterior_probs_logs

    def decode(self, S_e, S_i, posterior_probs_logs=None):
        T_data = S_e.shape[0]
        ...
        if posterior_probs_logs == None:
            ...
            Z_probs_out = torch.zeros(T_data, self.sub_no-1).cuda()
            ...
            for t in range(T_data):
                ...
                Z_logs = -F.softplus(X_out[1:] * self.W_spike + self.theta_spike)
                Z_probs = torch.exp(Z_logs)
                Z_ratios = Z_probs / (1-Z_probs)
                ...
                Z_probs_out[t] = Z_probs_out[t] + Z_probs
            ...
            return final_voltage, final_Y, final_Z_samples, Z_probs_out

        else:
            X = torch.zeros(T_data, self.sub_no).cuda() ### after first nonlinearity, before subunit scalar
            Y = torch.zeros(T_data+1, self.sub_no).cuda() ### after subunit scalar
            Z_prior_probs_logs_out = torch.zeros(T_data, self.sub_no-1).cuda()
            Z_post_probs_logs_out = torch.zeros(T_data, self.sub_no-1).cuda()
            Z_samples = torch.zeros(T_data + self.T_hist, self.sub_no - 1).cuda() ### spike trains
            ...
            for t in range(T_data):
                ...
                Z_probs_logs_prior = -F.softplus(X_out[1:] * self.W_spike + self.theta_spike)
                #Z_probs_logs_post = posterior_probs_logs[t]
                #Z_probs_logs_final = 0.5*Z_probs_logs_prior + 0.5*Z_probs_logs_post
                Z_probs_logs_final = posterior_probs_logs[t]
                
                Z_probs_final = torch.exp(Z_probs_logs_final)
                Z_ratios_final = Z_probs_final / (1-Z_probs_final)
                ...
                Z_post_probs_logs_out[t] = Z_post_probs_logs_out[t] + Z_probs_logs_final
                Z_prior_probs_logs_out[t] = Z_prior_probs_logs_out[t] + Z_probs_logs_prior
                Z_samples[t+self.T_hist] = Z_samples[t+self.T_hist] + Z_post
                spike_hist = torch.cat((spike_hist.reshape(-1,self.T_hist)[:,1:], Z_post.reshape(-1,1)),1)
            ...
            return final_voltage, final_Y, final_Z_samples, Z_prior_probs_logs_out, Z_post_probs_logs_out

    def forward(self, V_in, S_e, S_i, beta):

        posterior_probs_logs = self.encode(V_in, S_e, S_i)
        V_out, Y_out, Z_spikes, Z_logs_prior, Z_logs_post = self.decode(S_e, S_i, posterior_probs_logs)
        
        ### Reconstruction Loss ###
        #rec_loss = torch.mean((V_out - V_in)**2)
        rec_loss = torch.var(V_out - V_in)

        Z_P_prior = torch.exp(Z_logs_prior)
        Z_P_post = torch.exp(Z_logs_post)
