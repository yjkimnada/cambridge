...
class step_shGLM(nn.Module):
    def __init__(self, C_den, C_syn_e, C_syn_i, T_syn, syn_basis_no, T_hist,
        ...
        self.Theta_s = nn.Parameter(torch.rand(self.sub_no) * -0.2, requires_grad=True)
        ...
        self.W_s_prop = nn.Parameter(torch.rand(self.sub_no) * 0.02, requires_grad=True)
        ...
        self.spike_decay = nn.Parameter(torch.rand(self.sub_no) * 0.25, requires_grad=True)
    ...
    def forward(self, S_e, S_i):
        ...
        X_s_pad = torch.zeros(T_data+1, self.sub_no).cuda() ### sub-threshold values for spiking subunit
        ...
        for t in range(T_data):
            ...
            raw_ns_prop = torch.matmul(self.C_den, Y_ns_pad[t].clone()) # sub_no
            spike_ns_prop = torch.matmul(self.C_den[:,:], spike_ns_hist) # sub_no
            spike_ns_hist = spike_ns_hist.reshape(1, self.sub_no, -1)
            ...
            filtered_ns_hist = F.conv1d(spike_ns_hist, hist_ns_kern, groups=self.sub_no).flatten() # sub_no 
            ...
            X_s_in = X_s_pad[t].clone()*self.spike_decay + syn_s_in[t,:] + torch.matmul(self.C_den ,spike_s_last*self.W_s_prop) + self.Theta_s
