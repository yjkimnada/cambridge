...
class cont_shGLM(nn.Module):
    def __init__(self, C_den, C_syn, syn_T, Ensyn,
        ...
        self.thresh = nn.Parameter(torch.ones(self.sub_no)*(-2), requires_grad=True)
    ...
    def forward(self, X):
        ...
        for s in range(self.sub_no):
            ...
            pad_in_e = torch.zeros(X_T.shape[0]+self.syn_T-1, in_e.shape[1])
            pad_in_i = torch.zeros(X_T.shape[0]+self.syn_T-1, in_i.shape[1])
            pad_in_e[-X_T:,:] = pad_in_e[-X_T:,:] + in_e
            pad_in_i[-X_T:,:] = pad_in_i[-X_T:,:] + in_i
            pad_in_e = pad_in_e.T.unsqueeze(1)
            pad_in_i = pad_in_i.T.unsqueeze(1)
        ...
        ##############################
            ...
            for m in range(self.sub_no):
                subunit = self.sub_no - 1 - m
                spike_hist = torch.zeros(self.hist_T + self.hist_T - 1).cuda()
                ...
                if subunit not in root_idx:
                    ...
                    for t in range(X_T):
                        sub_thresh_pre = sub_in[t] = sub_in[t] + F.conv1d(spike_hist.reshape(1,1,-1),
                                                                          full_kern.reshape(1,1,-1), padding=0)
