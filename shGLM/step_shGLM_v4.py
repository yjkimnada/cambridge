...
class step_shGLM(nn.Module):
    def __init__(self, C_den, C_syn_e, C_syn_i, T_syn, syn_basis_no, T_hist,
        ...
        self.W_ns_hist = nn.Parameter(torch.rand(self.hist_basis_no) * (-0.02), requires_grad=True)
        ...
        self.Delta_ns_hist = nn.Parameter(torch.rand(1), requires_grad=True)
    ...
    def forward(self, S_e, S_i):
        ...
        Y_ns_pad = torch.zeros(T_data+1, self.sub_no).cuda() ### scaled ancestor subunit inputs to spiking subunit
        ...
        for t in range(T_data):
            ...
            filtered_ns_hist = F.conv1d(root_spike_hist, hist_ns_kern, groups=1) # sub_no

            X_ns_in = syn_ns_in[t] + raw_ns_prop + self.Theta_ns # sub_no
            X_s_in = syn_s_in[t] + filtered_s_prop + self.Theta_s  + filtered_s_hist
            X_ns_in[0] = X_ns_in[0] + filtered_ns_hist
