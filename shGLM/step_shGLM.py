...
class step_shGLM(nn.Module):
    def __init__(self, C_den, C_syn_e, C_syn_i, T_syn, syn_basis_no, T_hist,
        ...
        self.spike_derivScale = torch.ones(self.sub_no - 1).cuda() * 2
        self.spike_derivTime = torch.ones(self.sub_no - 1).cuda() * 0.25
        self.spike_threshold = nn.Parameter(torch.zeros(self.sub_no - 1), requires_grad=True)
    ...
    def forward(self, S_e, S_i):
        ...
        for t in range(T_data):
            ...
            X_s_out = torch.matmul(remove_first_sub, X_s_in)
            Z_out = Spike_Function.apply(X_s_out, self.spike_threshold, self.spike_derivScale, self.spike_derivTime)
...
class Spike_Function(torch.autograd.Function):
    @staticmethod
    def forward(ctx, input_value, threshold, derivScale, derivTime):
        ...
        for x in range(input_value.shape[0]):
            if input_value[x] < threshold[x]:
                output_value[x] = 0
            else:
                output_value[x] = 1

        threshold = torch.autograd.Variable(threshold, requires_grad=False)
        ...
        ctx.save_for_backward(input_value, threshold, derivScale, derivTime)

        return output_value
    
    def backward(ctx, gradOutput):

        (input_value, threshold, derivScale, derivTime) = ctx.saved_tensors
        spikeDerivative = derivScale * torch.exp( -torch.abs(input_value - threshold) / derivTime)
