...
class gumbel_shGLM(nn.Module):
    def __init__(self, C_den, C_syn_e, C_syn_i, T_syn, syn_basis_no, T_hist,
        ...
        ### Synapse Parameters ###
        self.W_s_syn = nn.Parameter(torch.randn(self.sub_no, self.syn_basis_no, 2) * 0.5, requires_grad=True)
        ...
        ### History Parameters ###
        self.W_s_hist = nn.Parameter(torch.rand(self.sub_no - 1, self.hist_basis_no) * (-1), requires_grad=True)
        self.W_ns_hist = nn.Parameter(torch.rand(self.sub_no - 1, self.hist_basis_no) * (-0.1), requires_grad=True)
        self.Tau_s_hist = nn.Parameter(torch.arange(self.hist_basis_no).float() , requires_grad=True)
        self.Tau_ns_hist = nn.Parameter(torch.arange(self.hist_basis_no).float() , requires_grad=True)
        ...
        self.W_s_prop = nn.Parameter(torch.rand(self.sub_no , self.prop_basis_no) * 0.5, requires_grad=True)
        self.W_ns_prop = nn.Parameter(torch.rand(self.sub_no , self.prop_basis_no) * 0.1, requires_grad=True)
        self.Tau_s_prop = nn.Parameter(torch.arange(self.prop_basis_no).float() , requires_grad=True)
        self.Tau_ns_prop = nn.Parameter(torch.arange(self.prop_basis_no).float() , requires_grad=True)
    ...
    def forward(self, S_e, S_i):
        ...
        Y_s_pad = torch.zeros(T_data+1, self.sub_no - 1).cuda() ### scaled ancestor subunit inputs to spiking subunit
        ...
        for t in range(T_data):
            raw_s_prop = torch.matmul(self.C_den[1:,1:], Y_s_pad[t]) # sub_no -1
            ...
            filtered_s_prop = torch.matmul(remove_first_sub, F.conv1d(spike_prop, prop_s_kern, groups=self.sub_no).flatten())
            ...
            X_s_in = syn_s_in[t,1:] + filtered_s_prop + raw_s_prop + self.Theta_s + filtered_s_hist # sub_no - 1
            X_ns_in[1:] = X_ns_in[1:] + filtered_ns_hist
            X_ns_out = torch.sigmoid(X_ns_in)
            Y_ns_pad[t+1] = X_ns_out * self.W_ns_sub
            Y_s_pad[t+1] = X_ns_out[1:] * self.W_s_sub

            X_s_out = torch.sigmoid(X_s_in)
