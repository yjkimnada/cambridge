...
class gumbel_NS_shGLM(nn.Module):
    def __init__(self, C_den, C_syn_e, C_syn_i, T_syn, syn_basis_no, T_hist,
        ...
        self.W_s_s_sub = nn.Parameter(torch.rand(self.sub_no - 1) , requires_grad=True)
        self.W_s_ns_sub = nn.Parameter(torch.rand(self.sub_no - 1) , requires_grad=True)
    ...
    def forward(self, S_e, S_i):
        ...
        #Y_s_slice = Y_s_pad[0]
        #Y_ns_slice = Y_ns_pad[0]
        Y_s_slice_NEW = Y_s_pad[0]
        Y_ns_slice_NEW = Y_ns_pad[0]
        raw_s_s_prop = Y_s_pad[0]
        raw_s_ns_prop = Y_ns_pad[0]
        raw_ns_s_prop = Y_s_pad[0]
        raw_ns_ns_prop = Y_ns_pad[0]

        for t in range(T_data):
            raw_s_s_prop = torch.matmul(Y_s_slice_NEW.clone()*self.W_s_s_sub, self.C_den[1:,1:].T) # sub_no - 1
            raw_s_ns_prop = torch.matmul(Y_s_slice_NEW.clone()*self.W_s_ns_sub, self.C_den[:,1:].T) # sub_no
            raw_ns_s_prop = torch.matmul(Y_ns_slice_NEW.clone()*self.W_ns_s_sub, self.C_den[1:,:].T) # sub_no - 1
            raw_ns_ns_prop = torch.matmul(Y_ns_slice_NEW.clone()*self.W_ns_ns_sub, self.C_den.T) # sub_no
                
            Y_s_slice_NEW = torch.sigmoid(syn_s_in[t,1:] + raw_s_s_prop + raw_ns_s_prop + self.Theta_s)
            Y_ns_slice_NEW = torch.sigmoid(syn_ns_in[t,:] + raw_s_ns_prop + raw_ns_ns_prop + self.Theta_ns)
            
            #Y_s_slice = Y_s_slice - Y_s_slice + Y_s_slice_NEW
            #Y_ns_slice = Y_ns_slice - Y_ns_slice + Y_ns_slice_NEW
            Y_s_pad[t+1] =  Y_s_slice_NEW
            Y_ns_pad[t+1] =  Y_ns_slice_NEW
