...
class shGLM(nn.Module):
    def __init__(self, C_den, C_syn, W_size, Ensyn,
                 syn_basis_tau, hist_basis_tau, spike_status, hist_T):
        ...
        self.C_syn = C_syn
        self.Ensyn = Ensyn
        self.syn_basis_tau = syn_basis_tau
        self.hist_basis_tau = hist_basis_tau
        self.syn_basis_no = syn_basis_tau.shape[0]
    ...
    def forward(self, X):
        ...
        t_e = torch.arange(self.W_size).cuda()
        t_i = torch.arange(self.W_size).cuda()
        ...
        for s in range(self.sub_no):
            ...
            full_e_kern = torch.zeros(self.W_size).cuda()
            full_i_kern = torch.zeros(self.W_size).cuda()

            for b in range(self.syn_basis_no):
                tau = self.syn_basis_tau[b]
                t_e_tau = t_e / tau
                t_i_tau = t_i /tau
            ...
            filtered_e = F.conv1d(in_e, full_e_kern, padding=int(self.W_size//2))
            filtered_i = F.conv1d(in_i, full_i_kern, padding=int(self.W_size//2))
            filtered_e = filtered_e.squeeze(1).T
            filtered_i = filtered_i.squeeze(1).T
            #subunit_in[:,s] = torch.sum(filtered_e * self.W[s,0], 1) + torch.sum(filtered_i * self.W[s,1], 1)
        ...
        #print(torch.mean(subunit_in, 0))
        #print(torch.mean(subunit_out, 0))
        #print(torch.mean(sub_out, 0))
        #print(torch.std(sub_out, 0))
        #print(torch.mean(feedback_sub_out,0))
        #print(torch.std(feedback_sub_out, 0))
        #print(spike_hist)
